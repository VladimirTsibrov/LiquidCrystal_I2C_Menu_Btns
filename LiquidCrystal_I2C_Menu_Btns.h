#ifndef TSBR_LIQUID_CRYSTAL_I2C_MENU_BTNS_H
#define TSBR_LIQUID_CRYSTAL_I2C_MENU_BTNS_H
...
/**
   ...
   most cases.
*/
class LiquidCrystal_I2C_Menu_Btns : public Print {
  public:
    /**
       Constructor
    ...
    */
    LiquidCrystal_I2C_Menu_Btns(uint8_t lcd_addr, uint8_t lcd_cols, uint8_t lcd_rows, uint8_t charsize = LCD_5x8DOTS);
    ...
    */
    void attachButtons(int8_t, int8_t, int8_t, int8_t = -1);
...
template <typename T> T LiquidCrystal_I2C_Menu_Btns::inputVal(const String &title, T minValue, T maxValue, T defaultValue, T step, void (*onChangeFunc)(T)) {
  return inputVal(title.c_str(), minValue, maxValue, defaultValue, step, onChangeFunc);
}

template <typename T> T LiquidCrystal_I2C_Menu_Btns::inputVal(const char title[], T minValue, T maxValue, T defaultValue, T step, void (*onChangeFunc)(T)) {
  T v = defaultValue;
...
template <typename T> T LiquidCrystal_I2C_Menu_Btns::inputValAt(uint8_t x, uint8_t y, T minValue, T maxValue, T defaultValue, T step, void (*onChangeFunc)(T)) {
  T v = defaultValue;
  if ((minValue > maxValue) or (v < minValue) or (v > maxValue))
    return v;
...
template <typename T> bool LiquidCrystal_I2C_Menu_Btns::inputValBitwise(const String &title, T &value, uint8_t precision, uint8_t scale, bool _signed) {
  return inputValBitwise(title.c_str(), value, precision, scale);
}

template <typename T> bool LiquidCrystal_I2C_Menu_Btns::inputValBitwise(const char title[], T &value, uint8_t precision, uint8_t scale, bool _signed) {
  char buffer[_cols];
  String S;
  int dotPos;
  if (value)
...
#endif // TSBR_LIQUID_CRYSTAL_I2C_MENU_BTNS_H
